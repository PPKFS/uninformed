...
newtype LexerSourceLocation = LexerSourceLocation
  { _lexNodeSpan :: ((Int, SourcePos), (Int, SourcePos))
...
data Lexeme = Lexeme
  { _tokLoc :: LexerSourceLocation
  , _tokType :: TokenType
  } deriving stock (Eq, Show)

instance Display Lexeme where
  displayBuilder (Lexeme _ t) = displayBuilder t
...
data PunctuationToken =
  ...
  | ParenOpen
  | ParenClose
  | ForwardSlash deriving stock (Eq, Enum, Show)
...
beginOrEndLiteralMode = --if we're in literal mode, reading a speech mark takes us out
  (guardM (use lexLiteralMode) >> single '"' >> lexLiteralMode .= False >> return StringEnd)
  <|> (guardM (not <$> use lexLiteralMode) >> single '"' >> lexLiteralMode .= True >> return StringBegin)
...
wordTok :: 
  (MonadParsec e Text m)
  => m TokenType
wordTok = Word <$> takeWhile1P Nothing isAlphaNum
...
annotateToken p = do
  ...
  a <- getSourcePos
  a' <- getOffset
  return (LexerSourceLocation ((b', b), (a', a)), r)
...
tokenLength :: 
  Lexeme 
  -> Int
tokenLength l = fst (lexemeEnd l) - fst (lexemeStart l)

lexemeStart :: 
  Lexeme 
  -> (Int, SourcePos)
lexemeStart (Lexeme (LexerSourceLocation (a, _)) _) = a

lexemeEnd :: 
  Lexeme 
  -> (Int, SourcePos)
lexemeEnd (Lexeme (LexerSourceLocation (_, a)) _) = a
